...
"""api.py:
Python API for the sensor toolbox server.
The library allows you to simply call methods to control remote sensors, as if they are local python objects.
...
class RemoteSensor():
    def __init__(self, config_path: str | None = None) -> None:
        ...
        #   stage states
        lg.info("Updating local parameter state from remote...")
        self.parameter = self.rest_get_parameter()
        lg.info("Current parameter of sensor: {}".format(self.parameter))
        self.sensor_parameter_state_watchdog_running = False
        self.sensor_parameter_state_watchdog_thread = threading.Thread(
            target=self.__sensor_parameter_state_watchdog_task)
    ...
    def initiate_watchdogs(self) -> None:
        ...
        self.sensor_parameter_state_watchdog_running = True
        self.sensor_parameter_state_watchdog_thread.start()
    ...
    def close_watchdogs(self) -> None:
        ...
        self.sensor_parameter_state_watchdog_running = False
        self.sensor_parameter_state_watchdog_thread.join()
    ...
    def __sensor_parameter_state_watchdog_task(self):
        """
        Checks for remote availability and sensor parameters constantly.
        """
        while self.sensor_parameter_state_watchdog_running:
            # poll for current parameters state every 5 seconds, 
            # to keep up with server if other clients changed server states
            self.parameter: self.rest_get_parameter()
            time.sleep(5.0)
    ...
    def handle_websocket_message(self, message: str):
        try:
            ...
            if "temperature" in r:
                lg.debug("Example WebSocket Protocol Handler: Received temperature data: {}".format(r["temperature"]))
    ...
    def close(self):
        lg.info("Halting connection to remote sensor.")
        self.close_watchdogs()
        self.close_websockets()
        lg.info("Closed all connection to remote sensor cleanly.")
    ...
    def check_websocket_availability(self) -> bool:
        ...
        try:
            ...
            lg.warning(
                "WebSocket for sensor is not ready yet!")
        except Exception as e:
            lg.error(
    ...
    def rest_get_parameter(self) -> int:
        return self.restful_get("parameter")

    def rest_get_data(self) -> int:
        return self.restful_get("data")
    ...
    def rest_set_temperature_sampling_interval(self, value: int, unit: str = 'ms'):
        payload = {"temperature_sampling_interval": {
            "value": value, "unit": unit}}
        return self.restful_post(resource_name="parameter", body=payload)

    def rest_set_humidity_sampling_interval(self, value: int, unit: str = 'ms'):
        payload = {"humidity_sampling_interval": {
            "value": value, "unit": unit}}
        return self.restful_post(resource_name="parameter", body=payload)

    def rest_start_continuous_sampling_mode(self):
        payload = {"continuous_sampling_mode": True}
        return self.restful_post(resource_name="parameter", body=payload)

    def rest_stop_continuous_sampling_mode(self):
        payload = {"continuous_sampling_mode": False}
        return self.restful_post(resource_name="parameter", body=payload)

    def websocket_command(self, command: dict, cid: int | None = None, timeout: float | None = None) -> int:
        """
        Send a command to the given sensor. Returns the ID of the command.
        Optional:
