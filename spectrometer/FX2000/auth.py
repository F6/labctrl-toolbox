...
"""auth.py:
This module provide functions for password verification, token generation, token validation and user access level validation.
"""
...
# own package
from .server_config import server_config, UserConfig, UserAccessLevel
...
def create_access_token(user: UserConfig, expires_delta: timedelta | None = None) -> str:
    ...
    jwt_config = server_config.auth.jwt
...
def check_access_level(level: UserAccessLevel, level_required: UserAccessLevel):
    if level < level_required:
        raise HTTPException(
            status_code=403, detail="User with access level '{}' is not allowed to perform this operation".format(level)
        )


def check_access_level_ws(level: UserAccessLevel, level_required: UserAccessLevel):
    if level < level_required:
        raise AccessLevelException(
            detail="User with access level '{}' is not allowed to perform this operation".format(
                level)
        )
